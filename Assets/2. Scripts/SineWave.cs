...
public class SineWave : MonoBehaviour
{
    ...
    [Tooltip("Sin 파동 효과 강도 (0 = Perlin만 적용)")]
    ...
    {
        // Perlin Noise 기반 파도 + Sin 변조
        float noiseX = worldPos.x * frequency + noiseWalk * time * speed;
        float noiseZ = worldPos.z * frequency + Mathf.Sin(time * speed) * 0.5f;

        float perlinValue = Mathf.PerlinNoise(noiseX, noiseZ);

        // Sin 함수로 추가적인 파동 효과
        float waveModulation = Mathf.Sin(time + worldPos.x * frequency + worldPos.z * frequency) * waveAmplitude;
